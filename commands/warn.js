...
module.exports = {
    ...
    async run (client, message, args) {
        ...
        if(warnings === 5) return message.channel.send(`${user} has reached the max amount of warnings.(5)`);


        if(warnings === null) {
            db.set(`warnings_${message.guild.id}_${user.id}`, 1);
            user.send(`You were warned in ${message.guild.name} for the reason of: \`${reason}\``)
